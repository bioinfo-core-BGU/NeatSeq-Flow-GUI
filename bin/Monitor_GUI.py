    ...
    def get_qstat(self,ssh_client):
        ...
        if len(xml)>0:
            ...
            if Executor=="PBS":
                ...
                # extract the jobs status
                state             = [PBS_STATUS_CODE[re.sub("job_state = ","",x)] for x in re.findall('job_state = \S+',xml)]
                if len(state)!=len(Job_name):
                    pass
                qstat["Job name"] = Job_name
                qstat["State"]    = state
    ...
    def read_run_log(self,ssh_client,runlog_file,Bar_len,Bar_Marker,Bar_Spacer,q=None,Instance=True,read_from_disk=True):
        ...
        try:
            ...
            # for the sample window information:
                ...
                self.rowmode=list(map(lambda x,y: 3 if y=="running" else x,self.rowmode,self.items['Running?']))
                ...
                if "Status" in runlog_Data.columns:
                    ...
                    self.items['Status'] = list(map(lambda x,y: y if ('ERROR' not in x ) and (y!="No")  else x, self.items['Status'],self.items['Running?']))
                
                self.items['Running?']=[x  if x=="running" else "No" for x in self.items['Running?']]
                if q!=None:
                    if self.session.status!=0:
    ...
    def Get_Command_Error(self,Step,STEP_format,SAMPLE_format,Samples,directory,scripts_index_file_loc,log_id,master_scripts_file_loc,qsub_names_file_loc,queue=None):
        ...
        if self.ssh_client!=None:
            ...
            if script_Step!='':
                ...
                jsonData   = json.load(open(os.path.join(directory , qsub_names_file_loc ), encoding="utf-8") )
                AllSamples = list(map(lambda x: x.split("..")[2],jsonData[Step]["low_qsubs"]))
                Samples    = list(set(AllSamples) - set(Samples))
...
class Monitor_GUI(flx.PyComponent):
    ...
    def init(self,directory         = os.getcwd(),
        ...
        self.NSF_exec                = 'scripts/NSF_exec.sh'
        global LOCAL_HOST
        global Executor
        ...
        if self.ssh_client !=None:
            ...
            try:
                ...
                if exit_status==0:
                    ...
                    SSH = Popen_SSH(self.session,ssh_client,'qstat')
                    [out, errs , exit_status] = SSH.output()
                    SSH.kill()
                    if exit_status==0:
                        Executor = "PBS"
            ...
            try:
                if subprocess.call('type qstat', shell=True,
                                        stdout=subprocess.PIPE, stderr=subprocess.PIPE) == 0:
                    if subprocess.call('qstat -xml', shell=True,
                                        stdout=subprocess.PIPE, stderr=subprocess.PIPE) == 0:
                        Executor = "SGE"
                    else:
                        Executor = "PBS"
                ...
                if ev.new_value!=self.step_rerun_heder+HEDER:
                    try:
                        ...
                        if Executor == "SGE":
                            ...
                            command = "grep " + step + ' ' +  os.path.join(self.directory ,self.master_scripts_file_loc) + " | grep " + self.NSF_exec
                            if self.ssh_client!=None:
                                SSH = Popen_SSH(self.session,self.ssh_client,command)
                                [out, errs , exit_status]  = SSH.output()
                                SSH.kill()
                        ...
                        if Executor == "SGE":
                            ...
                            if self.ssh_client!=None:
                                ...
                                if exit_status==0:
                                    ...
                                    out.remove("")
                                    if len(out)>0:
                                        script_Step = out[-1].split('\t')[-1]
                                if script_Step!="":
                                    command_Sample = "grep " + Sample + " " + script_Step + " | grep " + self.NSF_exec 
                                    SSH = Popen_SSH(self.session,self.ssh_client,command_Sample)
                                    [out, errs , exit_status]  = SSH.output()
                                    SSH.kill()
                                    if len(out)>0:
