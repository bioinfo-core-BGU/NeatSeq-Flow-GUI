...
from neatseq_flow_gui.modules.parse_sample_data import remove_comments, check_newlines
...
def get_param_data_YAML(filelines):
    ...
    def convert_param_format(param_dict):  # Gets the step part of the params dict
        ...
        for yamlname in yamlnames:
            ...
            if "redirects" in list(endparams[param_dict[yamlname]["module"]][yamlname].keys()):
            ...
            endparams[param_dict[yamlname]["module"]][yamlname]["redir_params"] = \
                {str(key):value \
                for (key,value) \
                in list(endparams[param_dict[yamlname]["module"]][yamlname]["redir_params"].items())}

            # Converting base to list if it is not one already
            if "base" in list(endparams[param_dict[yamlname]["module"]][yamlname].keys()) and not isinstance(endparams[param_dict[yamlname]["module"]][yamlname]["base"],list):
    ...
    step_order = list(yaml_params["Step_params"].keys())
    ...
    if "module_path" in param_data["Global"]:
        ...
        #=======================================================================
        bad_paths = [x for x in param_data["Global"]["module_path"] if not os.path.isdir(x)]
        good_paths = [x for x in param_data["Global"]["module_path"] if os.path.isdir(x)]
        if bad_paths:
    ...
    yaml_params["Global_params"] = param_data["Global"]
    return yaml_params
...
def parse_global_param_data_lines(g_filelines):
    ...
    for param in list(g_params.keys()):
    ...
    # pp(g_params)
    g_params = {param:(reduce(lambda x,y: " ".join([x,y]), val) if param in GLOBAL_PARAMS_MULTIPLE_V else val[0]) for param,val in g_params.items() if val != ['']}
    ...
    g_params = {param:(val[0] if (len(val) == 1) else val) 
                    for param,val in g_params.items()}


    # Check that Default_wait is integer. if passed:
    if "Default_wait" in list(g_params.keys()):
...
def parse_step_parameters(step_name,step_lines):
    ...
    if (len([sw_step_lines[ind][1] for ind in names_inds]) != len({sw_step_lines[ind][1] for ind in names_inds})):
        ...
        name_nums = [name for name in set(names) if len([x for x in names if x == name]) > 1]
...
def parse_name_parameters(sw_name_lines):
    ...
    redir_params = {param[1:]:(val[0] if len(val) == 1 else val) for param,val in redir_params.items() if val != []}
    ...
    # Remove singleton lists and empty lists (shouldnt exist)
    qsub_params = {re.sub("^qsub_opts_","",param):(val[0] if len(val) == 1 else val) for param,val in qsub_params.items() if val != []}
    ...
    other_params = {param:(val[0] if len(val) == 1 else val) for param,val in other_params.items() if val != []}
    ...
    if "base" in list(other_params.keys()):
...
def param_data_testing_global(param_data):
    ...
    if "Qsub_q" in list(param_data.keys()) and isinstance(param_data["Qsub_q"],list):
