...
def walk(node, variables_bunch, callback):
    ...
    if isinstance(node,dict):
        ...
        if len(node.keys()) == 1 and list(node.values())==[None] and re.match("Vars\.([\w\.]+?)",list(node.keys())[0]) :          
